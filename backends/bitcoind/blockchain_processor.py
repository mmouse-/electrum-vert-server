...
class BlockchainProcessor(Processor):
    ...
    def __init__(self, config, shared):
        ...
        while True:
            try:
                ...
                print_log('cannot connect to vertcoind...')
                time.sleep(5)
                continue
    ...
    def write_header(self, header, sync=True):
        ...
        with self.cache_lock:
            chunk_index = header.get('block_height')/240
    ...
    def getfullblock(self, block_hash):
        ...
        try:
            ...
            print_log("vertcoind error (getfullblock)")
            traceback.print_exc(file=sys.stdout)
            self.shared.stop()
        ...
        for ir in r:
            if ir['error'] is not None:
                self.shared.stop()
                print_log("Error: make sure you run vertcoind with txindex=1; use -reindex if needed.")
